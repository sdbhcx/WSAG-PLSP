...
class BiLevelRoutingAttention(nn.Module):
    ...
    def forward(self, x, ret_attn_mask=False):
        ...
        Return:
            NHWC tensor
        """
        # --- Step 1: Convert input to NHWC for window-wise operations
        # Expect original input in NCHW; this line rearranges to (n, h, w, c).
        ...
        # --- Step 3: Patchify into non-overlapping windows.
        # After this: x shape = (n, p^2, h, w, c) where p^2 = n_win * n_win windows.
        ...
        # --- Step 4: Project to queries and combined keys/values.
        # q: per-pixel queries; kv: concatenated per-pixel keys and values.
        q, kv = self.qkv(x) 

        # pixel-wise qkv
        # q_pix: (n, p^2, w^2, c_qk)
        # kv_pix: (n, p^2, h_kv*w_kv, c_qk+c_v)
        # --- Step 5: Prepare pixel-level tensors.
        # q_pix: flattened tokens per window -> (n, p^2, w^2, c_qk)
        # kv_pix: optionally downsampled k/v per window -> (n, p^2, h_kv*w_kv, c_kq+c_v)
        q_pix = rearrange(q, 'n p2 h w c -> n p2 (h w) c')
        kv_pix = self.kv_down(rearrange(kv, 'n p2 h w c -> (n p2) c h w'))
        kv_pix = rearrange(kv_pix, '(n j i) c h w -> n (j i) (h w) c', j=self.n_win, i=self.n_win)

        # --- Step 6: Compute window-level pooled q/k for routing (cross-window similarity)
        q_win, k_win = q.mean([2, 3]), kv[..., 0:self.qk_dim].mean([2, 3]) # window-wise qk

        ##################side_dwconv(lepe)##################
        # NOTE: call contiguous to avoid gradient warning when using ddp
        # --- Step 7: Side depthwise conv (LEPE) on value tokens to inject local positional features
        lepe = self.lepe(rearrange(kv[..., self.qk_dim:], 'n (j i) h w c -> n c (j h) (i w)', j=self.n_win, i=self.n_win).contiguous())
        lepe = rearrange(lepe, 'n c (j h) (i w) -> n (j h) (i w) c', j=self.n_win, i=self.n_win)

        ############ gather q dependent k/v #################

        # --- Step 8: Routing - select top-k related windows per query window.
        # r_idx: indices of selected windows, r_weight: corresponding routing weights.
        r_weight, r_idx = self.router(q_win, k_win) # both are (n, p^2, topk) tensors

        # --- Step 9: Gather selected k/v from the chosen top-k windows for each query window.
        # kv_pix_sel shape: (n, p^2, topk, h_kv*w_kv, c_kq+c_v)
        ...
        # --- Step 10: Rearrange tensors for multi-head attention computation.
        # Convert to (batch_windows, heads, seq_len, head_dim) layout.
        ...
        q_pix = rearrange(q_pix, 'n p2 w2 (m c) -> (n p2) m w2 c', m=self.num_heads) # to BMLC

        # param-free multihead attention
        # --- Step 11: Compute attention weights and aggregated output over gathered values.
        attn_weight = (q_pix * self.scale) @ k_pix_sel
        attn_weight = self.attn_act(attn_weight)
        out = attn_weight @ v_pix_sel
        # --- Step 12: Restore window layout to full feature map and add LEPE.
